...
#   revup-media-guard.timer       every 10 min     (one-way audio / no inbound RTP watchdog)
...
chmod +x "${SRC_DIR}"/pg_dump_nightly.sh "${SRC_DIR}"/cdr_archive_monthly.sh "${SRC_DIR}"/slot_wal_guard.sh "${SRC_DIR}"/asr_guard.sh "${SRC_DIR}"/media_guard.sh "${SRC_DIR}"/replication_guard.sh
systemctl daemon-reload
say "units installed to /etc/systemd/system"

# --- Enable timers -----------------------------------------------------------
systemctl enable --now revup-pgdump.timer revup-cdr-archive.timer revup-slot-wal-guard.timer revup-asr-guard.timer revup-media-guard.timer revup-replication-guard.timer
