...
- **A `--type=immediate` restore + `pg_promote` leaves `recovery_target=immediate` / `recovery_target_action=pause` / `restore_command` in `postgresql.auto.conf`, and `pg_promote` does NOT remove them.** Harmless on the restored primary, but any `pg_basebackup` clone taken from it INHERITS those lines → the new standby recovers to consistency then **pauses** (never streams) while spamming `archive-get requires pg1-path`. Strip them on every clone: `sudo -u postgres sed -i -E '/^[[:space:]]*(recovery_target|restore_command)/d' /var/lib/postgresql/16/main/postgresql.auto.conf`; clean the primary with `ALTER SYSTEM RESET recovery_target` / `recovery_target_action` / `recovery_target_timeline` / `restore_command` run as **separate** `psql -c` calls (chaining several `ALTER SYSTEM` in one `-c` errors `cannot run inside a transaction block`).
- **Replication slots are NOT in pgBackRest backups.** After a restore the primary comes back with zero slots — recreate each (`SELECT pg_create_physical_replication_slot('east_standby')`, …) before re-cloning standbys, or they can't attach.
- **Guard destructive VM commands with a hostname check** so a wrong-SSH-session paste can't fire them: `H=$(hostname); if [ "$H" = "TARGET" ]; then <destructive cmd>; else echo REFUSED; fi`. This exact class of mistake is what caused the 2026-07-22 incident below.
...
| 2026-07-22 | Keegan | **REAL incident** (scenario B) — prod `services` data dir wiped by an accidental `find … -delete` pasted into the wrong SSH session | **~4min** (`--type=immediate` restore + `pg_promote`) | **0 loss** (DB idle since the 15:27 backup) | ✅ Restored in place into the emptied data dir (Debian `/etc` config survived the delete) → `pg_promote` → read-write primary on timeline 3, `pgbackrest check` green. Both standbys re-cloned afterward (hit the `recovery_target`-inheritance gotcha above). The whole Phase-0/1 safety net proved out end-to-end under real fire. |
