...
| **FS-2 (HOT STANDBY, NEW)** | `east-fs-2` (192.168.10.3 · ext **35.196.226.123**) | `west-fs-2` (192.168.20.3 · ext **35.197.95.171**) | `central-fs-2` (192.168.30.3 · ext **34.63.100.161**) |
...
4. 🟠 **The step-2 call DROPS.** Expected — FS-1 was its media anchor. Log it in §9 ("established call lost — by design"). Also expect the **"FreeSWITCH DOWN on west-fs" `revup-fs-watchdog` page within ~2-3 min** (§8 — you killed the container; that is the watchdog working) — acknowledge it; a recovery INFO line follows after step 7.
...
3. 🟢 Read the pages: **"VM down" fired** → FS-1 VM is dead/stopped — GCP console. **"FreeSWITCH DOWN on {host}" fired (no VM-down)** → container-level FS-1 death (`revup-fs-watchdog`, §8): on FS-1, `sudo docker ps -a --filter name=voip-freeswitch` + `sudo docker logs --tail 100 voip-freeswitch`. **NO page but FS-2 is serving** → check the watchdog itself on FS-1 (`systemctl status revup-fs-watchdog.timer`) — it should have paged.
...
| **FS-1 container dead/hung, VM up** | **"FreeSWITCH DOWN on {host}" `revup-fs-watchdog` page ~2-3 min (§8)**; Grafana FS-2 serving + FS-1 `freeswitch_esl_scrape_ok` gone; customer redials succeed | Same flip/failback | Same as above | Hostname-guarded orphan-gotcha + `up -d` on FS-1 (§3 step 7) |
...
| **FS-2 dead while FS-1 serving** | "VM down" (VM-level) or the `revup-fs-watchdog` page (container-level — §8); Grafana FS-2 exporter series gone | None — no traffic moves | None | Restore promptly — you are one failure from a zone outage |
...
| `revup-fs-watchdog` timer — each media VM, 60s tick (`scripts/fs-watchdog/`, install below) | FS container dead OR ESL unresponsive on that VM → one `revup-alert` page (transition-based) + INFO recovery line | ~2-3 min (2 failed ticks + log-match) | Container-level FS death on EITHER node — closes the former gap |
...
**GAP CLOSED (2026-09-01 — `scripts/fs-watchdog/`, deploy below):** container-level FS death now pages. Each media VM runs `revup-fs-watchdog.timer` (60s tick): check 1 = `voip-freeswitch` container running (`docker inspect`); check 2 = ESL answering (`fs_cli -p $ESL_PASSWORD -x status` INSIDE the container — the secret stays in the container env, never in host logs). On the 2nd consecutive failed tick (~2 min) it emits ONE `revup-alert` syslog line (hostname + which check failed) → Ops Agent → the existing GCM log-match paging policy (rate-limited 30m; zero new GCP resources). Transition-based state in `/run/revup/fs-watchdog.state`: no re-page while still down, and an INFO `revup-fs-watchdog` recovery line (NOT the alert tag) on return to healthy. Catches BOTH nodes — a dead FS-2 standby (failure-mode row 4) pages the same way. **Expect this page during §3 drills** (you killed the container) — acknowledge it like the SBC-drill page. Tunables (optional `/etc/revup/fs-watchdog.env`): `FS_WATCHDOG_FAIL_THRESHOLD` (2), `FS_WATCHDOG_TIMEOUT` (10s), `FS_WATCHDOG_CONTAINER` (voip-freeswitch). The dashboard twin is the Traffic Status "Active FS (media HA election)" row (3-state per zone on `freeswitch_esl_scrape_ok`).

**Install (per media VM — all 6: `fs-media-v2`, `east-fs-2`, `west-fs`, `west-fs-2`, `central-fs`, `central-fs-2`; single lines):**
1. 🟢 `cd /opt/revup && sudo git pull`
2. 🟢 `sudo /opt/revup/scripts/fs-watchdog/install_fs_watchdog.sh` (idempotent — installs units incl. the shared `revup-alert@` relay, enables the 60s timer; warns if the ops-agent is inactive)
3. 🟢 Verify: `systemctl list-timers --no-pager 'revup-fs-*' && sudo systemctl start revup-fs-watchdog.service && journalctl -u revup-fs-watchdog.service -n 10 --no-pager && sudo cat /run/revup/fs-watchdog.state` → `fails=0 paged=0` on a healthy VM
4. 🟠 Optional one-time end-to-end page test (fires a REAL page on the shared policy — do it on ONE VM only, warn the on-call first): `logger -p user.err -t revup-alert -- "TEST page from $(hostname) — fs-watchdog install"`
