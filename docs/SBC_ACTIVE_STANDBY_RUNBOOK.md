...
**Why established calls survive an SBC death:** in-dialog routing is STATELESS (`docker/kamailio/CLAUDE.md` §8.10, PR #71) — in-dialog requests (ACK/BYE/re-INVITE) are routed from information carried in the Route set (`;fs=` dispatch), not from per-SBC dialog state, so WHICHEVER SBC the VIP delivers to can relay them. Media never touches the SBCs (RTP is FS↔carrier). So when an SBC dies: audio continues uninterrupted, and hangups signal correctly through the survivor. **What is lost:** call setups in flight during the ~10-12 s flip (INVITE transactions living on the dead SBC). UDP INVITE retransmits landing on the standby can self-recover some of these; the rest fail and the caller redials. Since the teardown-hygiene change, the carrier's CANCEL for such a setup draws a `481` from the new active (flag-5 trusted sources: Bandwidth/Sinch, FS, SIPp) instead of a silent drop, so the ~32 s CANCEL retransmit train is gone. This "established survive, setups-in-flight lost" semantic is the industry-standard HA bar for SBC pairs.

**What lingers (gauges only — do NOT "clean up"):** dialog state is per-SBC (`db_mode=0`, no DMQ). Every dialog the ex-active SBC owned at the flip never sees its BYE there (it lands on the new active, which tears the call down statelessly), so on an ex-active whose Kamailio is STILL RUNNING, `kamcmd dlg.stats_active`, `kamailio_dialog_active_dialogs` and the `carrier_trunk` profile gauges over-read until those entries expire at `default_timeout=7200` (≤2 h). The calls themselves are over; expiry is silent (no BYE) by design. **NEVER run `kamcmd dlg.end_dlg` / `dlg.terminate_dlg` sweeps to clear them** — those commands SEND BYEs and would kill live calls still up through that dialog. Ghosts expire on their own. Details: `docker/kamailio/CLAUDE.md` §8.11.
...
2. 🟢 Establish a test call: dial `+16174544217` → `+17744045256`, confirm two-way audio, KEEP IT UP through the flip (REQUIRED — this is the "established call survived" evidence for §8). Pre-flip baseline on the primary: `sudo docker exec voip-kamailio kamcmd dlg.stats_active` (expect ≥1).
3. 🟠 **In-flight case first:** from a SECOND phone dial `+16174544217` and leave it RINGING (do not answer at `+17744045256`). While it rings, stop Kamailio on the PRIMARY (hostname-guarded; the guard makes a wrong-box paste a silent no-op): `hostname | grep -q '^poc-custom-voip$' && cd /opt/revup && sudo docker compose -f docker-compose.sbc.yml stop kamailio` — then hang up the ringing phone.
...
5. 🟢 Verify the ESTABLISHED call, AFTER the flip: audio still flowing on the step-2 call (it never touched the dead SBC's media — there is none). Hang it up and record ALL of the following in §8:
   - (a) Homer: the hangup is a single `BYE` → `200 OK` (BYE relays via the standby — §8.10 stateless in-dialog; no ~30 s zombie, no 481/408 burst).
   - (b) On the zone FS VM: `sudo docker exec voip-freeswitch sh -c '/usr/local/freeswitch/bin/fs_cli -p $ESL_PASSWORD -x "show channels count"'` → `0 total` within 60 s.
   - (c) On the NEW active (standby): `sudo docker exec voip-kamailio kamcmd dlg.stats_active` → 0 for this call (it never had the dialog). The ex-active's copy is a ghost that EXPIRES ≤2 h on any SBC whose Kamailio is still running (§0 — in this drill the primary was stopped, so its state is simply gone; the observable ghost is the STANDBY's at failback, step 8). Do NOT sweep it.
   - (d) In-flight case (step 3): Homer shows the carrier's `CANCEL` answered with a single `481 Call/Transaction Does Not Exist` from the standby — NOT a ~32 s CANCEL retransmit train. (New behavior; a silent drop here is a regression.)
6. 🟢 Verify NEW calls: dial `+16174544217` again → completes via the standby. Grafana top row: standby card goes green. KEEP THIS CALL UP through failback (it becomes the ghost-dialog evidence in step 8).
...
8. 🟢 Watch automatic failback (~10-12 s after Kamailio answers its fs-aware /healthz; NO manual step — `failover-ratio=0` + no drain reverts traffic the moment the primary is healthy): re-run the step-4 command until primary is HEALTHY again, then place one more test call and confirm the Grafana primary card re-greens within ~10 min (the panel window). Now hang up the step-6 call and repeat 5(a)+(b); then on the STANDBY (now ex-active, still running) `sudo docker exec voip-kamailio kamcmd dlg.stats_active` STILL counts that dialog — EXPECTED, it expires ≤2 h (§0); on the primary it is 0. Note the count in §8.
9. 🟢 Log it in §8 — every column, including the teardown-evidence ones.

**Expected alert behavior during this drill:** the **"[CRITICAL] {zone} primary SBC UNHEALTHY"** health-check-log page fires within ~1 minute of step 3 — that is the alerting working, not a problem; acknowledge it. The SIP VIP uptime page must NOT fire (the standby is serving). If the VIP page fires, the standby did not take over — stop and investigate. The incident auto-closes on rate-limit/auto-close, not on recovery — get-health and the Grafana row are recovery truth. The vmalert teardown alerts (§7: `SipByeFailureShare`, `FsStaleChannels`, `SbcDialogLeak`) must NOT fire from a drill — `SbcDialogLeak`'s 2h30m window deliberately outlives the 2 h ghost lifetime; if any of them fires, teardown is broken, not the drill.
...
5. 🟢 Confirm failback (§2 step 8) and spot-check teardown quality in Homer (no 481/408 bursts on BYEs; a lone `481` answering a CANCEL right at the flip is the expected in-flight case, §0). `dlg.stats_active` on the SBC that was active BEFORE the flip stays elevated up to 2 h (ghosts, §0 — leave them); on the CURRENT active it must track live calls.
...
|---|---|---|---|---|
| **Primary VM dead** (crash/stop/host event) | GCM "[CRITICAL] {zone} primary SBC UNHEALTHY" (health-check log, ~1 min) + "VM down" ~5 min; get-health; Grafana standby card | Both planes flip to standby in ~10-12 s | Established calls SURVIVE; setups in flight during the flip lost (UDP retransmits recover some; their CANCELs get 481 from the standby); new calls via standby. Established calls tear down statelessly on the standby; no dialog ghosts (the dead VM's state is gone) | Recover the VM (§4); failback automatic |
| **Kamailio dead/hung, VM up** (container OOM/crash, wedged process not accepting TCP) | GCM "[CRITICAL] {zone} primary SBC UNHEALTHY" ~1 min (no VM-down page — that's the tell it's container-level); Grafana row + get-health | Same ~10-12 s flip (fs-aware /healthz refused/timed out) | Same as above. If the process survives (hung, not dead), its owned dialogs linger as gauge-only ghosts ≤2 h — leave them (§0) | Restart the container (hostname-guarded, §4 step 4) |
...
| **Failback / maintenance drain with carrier transactions still in flight on the demoted SBC** (its VIP-sourced INVITE/CANCEL replies now land on the new active) | `peer hand-off:` INFO lines on the active; Homer shows the reply traced on both SBCs | None — both SBCs healthy | With `SBC_PEER_INTERNAL_IP` set on both: none — the active hands the raw reply to the demoted peer's tm (`docker/kamailio/CLAUDE.md` §8.12). Without it: the owner's INVITE times out → local 408 → alternate-PoP re-INVITE = duplicate ring | Nothing. If the peer is DOWN (its group 10 reads Inactive) replies fall through to the stateless via2 forward as before — verify `kamcmd dispatcher.list` group 10 is `AP` on both once it recovers |
...
| vmalert `SipByeFailureShare` (`docker/vmalert/rules/teardown.yml`, on `zone:kamailio_bye_replies:rate5m{class}`) | Non-2xx BYE replies >20% of BYEs for 10 m in a zone | ~10 m | Teardown broken: in-dialog dispatch regression, a stuck SBC, or a peer answering BYEs with 481/408. Check Homer for the BYE shape (§4 step 5); a single 481 on a CANCEL at a flip is NOT this |
| vmalert `FsStaleChannels` (`freeswitch_channels_stale` / `freeswitch_channel_max_age_seconds{freeswitch_node}`) | Any FS channel older than 2 h, for 15 m | ~15 m | A real orphan on FS (a BYE that never reached FS) — `show channels` on that node, confirm with Homer, `uuid_kill` only the confirmed orphan. Grafana Traffic Status "Oldest live channel" shows it |
| vmalert `SbcDialogLeak` (Kamailio dialogs − FS channels > 10 for 2h30m) | An SBC's dialog count exceeds the zone's live FS channels for longer than the 2 h ghost lifetime | ~2.5 h | A PERSISTENT dialog leak on the SBC (post-flip ghosts expire before this can fire — by design). Investigate the SBC's dialog handling; do NOT `dlg.end_dlg`-sweep (§0). Grafana divergence panels go amber earlier |

**How the three vmalert rows reach a human:** vmalert has no notifier; `revup-vmalert-pager.timer` on the services VM (`scripts/vmalert-pager/`, 60 s) polls `:8880/api/v1/alerts` and emits ONE `revup-alert` syslog line per newly-firing `alertname|zone` (→ the existing GCM log-match page), an INFO `revup-vmalert-pager` line on recovery, and one page if the vmalert API is unreachable for 5 polls. Install: `cd /opt/revup && sudo bash scripts/vmalert-pager/install_vmalert_pager.sh`. Not installed ⇒ the rows above are dashboard-only.
...
| Date | Zone | Type (planned §2 / kill §3 / real) | Flip time observed | Established call survived (audio through flip Y/N) | Post-flip teardown evidence (§2 step 5: a single BYE→200 Y/N · b FS channels 0 in ≤60 s Y/N · c ex-active `dlg.stats_active` ghost count + new-active 0) | In-flight CANCEL → single 481 (§2 step 5d) Y/N | Setups lost noted | Failback clean | Notes |
|---|---|---|---|---|---|---|---|---|---|
| | | | | | | | | | |

**Evidence gap:** the 2026-08-26/27 migration drills (§9, all three zones) were run but NEVER logged here — there is no recorded evidence that an established call survived or tore down cleanly. The next §2 drill per zone must fill every column above.
...
2. 🟠 PRIMARY SBC same change — its restart causes one real ~10-12 s flip to the standby and automatic failback (an unplanned mini-drill; watch §1 step 1 recover).
...
4. 🟢 Acceptance: test call completes; Homer shows the B-leg INVITE arriving at the signaling VIP and the 200 OK toward FS carrying `Record-Route: <sip:10.138.0.250:5060;r2=on;fs=5090;lr>`; far-end hangup draws a single BYE 200 OK (no retransmit storm); `sudo docker exec voip-kamailio kamcmd dlg.stats_active` on the CURRENT active returns to 0 (on an SBC that was active before the step-2 flip it returns to 0 only after ≤2 h — ghosts, §0).
