    ...
    <div style={root}>
      ...
      <div className="sidebar-offset" style={shell}>
        <div style={frame()}>
          ...
          {/* Right pane — active thread or a glass placeholder. Keyed by
              conversation id so switching threads REMOUNTS MessageThread with
              fresh state (loading/list/scroll) instead of resetting via effect. */}
          <div style={threadPane}>
            {selectedConversation ? (
              <MessageThread key={selectedConversation.id} conversation={selectedConversation} />
            ) : (
              <ChatPlaceholder
