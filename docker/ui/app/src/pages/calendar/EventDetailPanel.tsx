/**
 * Read-only event detail slide-over. v1 has no create/edit/delete — this panel
 * only displays. Hooks are declared unconditionally at the top (React #310);
 * the panel renders nothing when `event` is null but still runs its hooks.
 *
...
function rsvpMeta(
  ...
  switch (status) {
    ...
    case 'tentative':
      return { label: 'Maybe', color: '#f59e0b' };
    case 'needs_action':
  ...
  return (
    ...
    >
      ...
      >
              ...
              {event.status !== 'confirmed' && (
                <span
                  style={{
                    ...
                    color: event.status === 'cancelled' ? '#f87171' : '#f59e0b',
                  }}
                >
                  {event.status}
                </span>
          ...
          {joinUrl && (
            <a
              ...
              style={{
                ...
                fontWeight: 700,
                textDecoration: 'none',
                boxShadow: '0 2px 14px -4px rgba(45,212,191,0.6)',
              }}
            >
          ...
          {event.organizer && (event.organizer.display_name || event.organizer.email) && (
            <div>
              ...
              <div style={{ fontSize: '0.85rem', color: '#cbd5e0', lineHeight: 1.4 }}>
                ...
                {event.organizer.display_name && event.organizer.email && (
                  <span style={{ color: '#64748b', fontSize: '0.78rem' }}> · {event.organizer.email}</span>
                )}
              </div>
            </div>
          )}
        ...
        {event.html_link && (
          <div style={{ padding: '14px 22px', borderTop: '1px solid rgba(42,47,69,0.6)' }}>
            <a
