...
export function DidSection({ trunkId }: { trunkId: number }) {
  // The dropdown-wrapper DOM ref is owned HERE (the component that attaches
  // it) and handed to the hook for its click-outside listener. If the hook
  // returned the ref inside its result object, react-hooks/refs would treat
  // every `m.*` read during render as a ref access.
  const wrapperRef = useRef<HTMLDivElement>(null);
  const m = useDidManager(trunkId, wrapperRef);
