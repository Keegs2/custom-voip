/**
 * Content sections for the API Reference page. Pure presentation — each section
 * is a <DocsAccordion> filled with the docs primitives (text, code, endpoints,
 * tables, note cards). The page file composes these in order.
 *
 * Blue glass throughout; amber is reserved for "coming soon" / planned-feature
 * semantics only.
 */

import { Key, Code, Phone, Zap, Server } from 'lucide-react';
import { DocsAccordion } from './DocsAccordion';
import { P, H3, IC, Callout, B } from './text';
import { CodeBlock, ReqRes } from './code';
import { Endpoint, ParamTable, NoteCards } from './apiRefs';
import {
  DOCS,
  MONO,
  tableFrame,
...
export function ComingSoonBadge() {
  return <span style={comingSoonBadge()}>Coming Soon</span>;
}

/* ─── Section 1: Getting Started ────────────────────────── */

export function ApiGettingStartedSection() {
  return (
    <DocsAccordion
      accent={DOCS.accent}
      icon={<Key size={18} />}
    ...
    >
      <P>
        The Shale API is RESTful and JSON-based. Every request to a protected endpoint must include an{' '}
        <IC>Authorization</IC> header with a valid JWT bearer token. Tokens are obtained by POSTing credentials
        to the login endpoint and are valid for <B>8 hours</B>.
      </P>
  ...
  -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."`}
      ...
      <div style={endpointRow}>
        <Server size={16} style={{ color: DOCS.accent, flexShrink: 0 }} />
        <div>
...
curl "https://your-portal-url/api/v1/rcf" \
  -H "Authorization: Bearer $TOKEN"`}
      ...
      <H3>Rate limits</H3>
      <P>
        The API enforces a limit of <B>100 requests per minute</B> per token. Exceeding this limit returns{' '}
        <IC>429 Too Many Requests</IC> with a <IC>Retry-After</IC> header indicating how many seconds to wait.
        If you need a higher limit for bulk provisioning workflows, contact your Granite account team.
      </P>
  ...
  "detail": "RCF entry +17745554321 not found"
      ...
      <Callout>
        <B>Phone number format:</B> All phone numbers must be in E.164 format — a leading <IC>+</IC> followed by
        country code and subscriber number with no spaces or dashes. Example: <IC>+17745551234</IC>. When
        placing an E.164 number in a URL path segment, URL-encode the <IC>+</IC> as <IC>%2B</IC>. For example,{' '}
...
export function ApiRcfSection() {
  return (
    ...
    >
      <P>
        The RCF API lets you manage your forwarding numbers from any application or script. All endpoints are
        scoped to your customer account — you can only read and modify numbers belonging to your organisation.
        The identifier in update and delete endpoints can be either a numeric row ID or an E.164 DID
        (URL-encoded).
      </P>
  ...
  "updated_at": "2026-04-23T11:05:00Z"
      ...
      <P>
        The <IC>identifier</IC> path parameter accepts either a <B>numeric ID</B> or an <B>E.164 DID</B>
        (URL-encoded). This lets you update by DID without first fetching the numeric ID.
      </P>
...
# Returns: 204 No Content — no response body`}
      ...
      <Callout>
        DELETE is idempotent — if the DID does not exist, the API returns 204 regardless. It is always safe to
        retry a DELETE after a network timeout without checking for duplicates.
      </Callout>
    </DocsAccordion>
  );
}
...
export function ApiNumbersSection() {
  return (
    <DocsAccordion
      accent={DOCS.accent}
      icon={<Phone size={18} />}
    ...
    >
      <P>
        The number inventory API lets you discover available DIDs in specific area codes or cities, view
        numbers already assigned to your account, and submit reservation requests for admin review.
      </P>
...
# Returns: 201 Created when the reservation request is submitted`}
      ...
      <Callout>
        Number requests are reviewed by a Granite administrator before the DID is assigned to your account.
        Once approved, the number will appear in your RCF number list and you can configure forwarding via the
        portal or the API.
      </Callout>
    </DocsAccordion>
  );
...
const WEBHOOK_EVENTS = [
  { event: 'call.started', desc: 'Fires when an inbound call is received on a DID and forwarding begins. Includes caller number, DID, and forward_to destination.' },
  { event: 'call.answered', desc: 'Fires when the forwarding destination picks up. Includes answer timestamp and destination number.' },
  { event: 'call.ended', desc: 'Fires when a call disconnects. Includes call metadata and quality summary.' },
  { event: 'call.failover', desc: 'Fires when ring timeout expires and the call is redirected to the failover destination. Includes both primary and failover numbers.' },
];

const RETRY_ROWS: { status: string; retry: 'no' | 'reauth' | 'backoff'; action: string }[] = [
...
function retryPillStyle(retry: 'no' | 'reauth' | 'backoff') {
  const tone =
    retry === 'no'
      ? { color: DOCS.textFaint, bg: 'rgba(71,85,105,0.18)' }
      : retry === 'reauth'
        ? { color: '#c084fc', bg: 'rgba(168,85,247,0.12)' }
        : { color: DOCS.amber, bg: hexToRgba(DOCS.amber, 0.14) };
  return {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 24 }}>
        {QUICK_START.map(({ step, tag, title, body }) => (
          <div key={step} style={stepCard}>
            <div style={stepBadge()}>{step}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 5 }}>
                <div style={{ fontSize: '0.87rem', fontWeight: 700, color: DOCS.text }}>{title}</div>
                <span style={stepTag()}>{tag}</span>
              </div>
              <div style={{ fontSize: '0.82rem', color: DOCS.textMuted, lineHeight: 1.7 }}>{body}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <H3>
        Webhook integration <ComingSoonBadge />
      </H3>

      <Callout accent={DOCS.amber}>
        Webhook support is currently in development. The event schema below reflects the planned interface —
        subscribe to release notes for availability updates.
      </Callout>

      <P>
        Webhooks allow Shale to push call event data to your systems in real time rather than requiring your
        application to poll the CDR API. When enabled, the platform will POST to your registered HTTPS endpoint
        with a JSON payload for each of the following events:
      </P>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 10, marginBottom: 20 }}>
        {WEBHOOK_EVENTS.map(({ event, desc }) => (
          <div key={event} style={noteCard(DOCS.amber)}>
            <div style={{ fontFamily: MONO, fontSize: '0.75rem', fontWeight: 700, color: DOCS.amber, marginBottom: 8 }}>
              {event}
            </div>
            <div style={{ fontSize: '0.81rem', color: DOCS.textMuted, lineHeight: 1.6 }}>{desc}</div>
          </div>
        ))}
      </div>
      ...
      <P>
        Production integrations must account for transient failures and rate limits. The table below classifies
        which status codes are safe to retry and which require intervention.
      </P>
      ...
      <div style={tableFrame}>
        <table style={table}>
          ...
          <tbody>
            {RETRY_ROWS.map((row, i) => (
              <tr key={row.status} style={zebra(i)}>
                <td style={tdNoWrap}>
                  <code style={codeCell}>{row.status}</code>
                </td>
                <td style={tdNoWrap}>
                  <span style={retryPillStyle(row.retry)}>
      ...
      <P>
        For retryable errors, use exponential backoff with jitter to avoid thundering-herd effects when
        multiple instances of your integration retry simultaneously:
      </P>
...
def api_request_with_retry(method, url, **kwargs):
    ...
    raise RuntimeError(f"Request failed after {max_retries} attempts")`}
      ...
      <NoteCards
        accent={DOCS.accent}
        items={[
          { title: 'Check before create', body: 'Before POSTing a new DID, first GET /api/v1/rcf/{did} to check if it already exists. If you get 200, use PUT to update instead — POST will return 409 Conflict.' },
          { title: 'DELETE is always safe to retry', body: 'DELETE returns 204 whether or not the resource existed. Retrying a DELETE after a timeout is always safe — no error is returned if already deleted.' },
          { title: 'Token expiry strategy', body: 'Track token issue time in your integration. Re-authenticate proactively a few minutes before the 8-hour expiry rather than waiting for a 401.' },
          { title: 'Store token securely', body: 'Never commit tokens to source control. Use environment variables or a secrets manager. Treat a token with the same sensitivity as a password.' },
        ]}
      />

      <H3>
        Bulk provisioning <ComingSoonBadge />
      </H3>

      <Callout accent={DOCS.amber}>
        Bulk CSV provisioning is planned for a future release. The schema described here reflects the intended
        API design.
      </Callout>

      <P>
        For deployments with large DID inventories, the planned bulk endpoint accepts a CSV file with multiple
        RCF entries in a single API call. The platform will process the file asynchronously and return a job ID
        you can poll for status.
      </P>
...
+17745551003,7,+16175553001,Cambridge Office,false,30,+18005550000,true`}
      ...
      <P>
        All phone numbers must be in E.164 format. Boolean fields accept <IC>true</IC> or <IC>false</IC>. Leave{' '}
        <IC>failover_to</IC> empty to provision without a failover destination.
      </P>
