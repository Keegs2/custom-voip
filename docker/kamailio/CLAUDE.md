...
| `__SBC_PEER_IP__` + `__SBC_PEER_MODE_DEFINE__` (cfg), `__SBC_PEER_DISPATCHER_LINE__` (dispatcher.list) | `SBC_PEER_INTERNAL_IP` | unset (feature compiled out; token renders as `SBC_INTERNAL_IP`) | The OTHER SBC's VPC IP — peer reply hand-off (§8.12) + dispatcher group 10 liveness probe. Validated: dotted-quad IPv4 and `!= SBC_INTERNAL_IP`, else WARNING + treated as unset. |
...
8. **CANCEL**: `t_check_trans()` true (a matching INVITE transaction exists on THIS SBC) → RELAY, unchanged. False → `481 Call/Transaction Does Not Exist` if the source is flag-5 trusted (static carriers, FS/internal, SIPp), else silently dropped (never reflect replies to unauthenticated sources — trunk PBX / DB-carrier trust is per-INVITE and unknown on a bare CANCEL). The 481 case is the post-failover shape: the INVITE transaction lived on the SBC that died; 481 (RFC 3261 §9.2) stops the carrier's ~32s CANCEL retransmit train and the phantom ring on the forwarded leg. See §8.11.
...
Handles BYE, re-INVITE, ACK, UPDATE, etc. for established calls. (CANCEL never gets here — it is handled at the top of request_route via `t_check_trans()`: RELAY, or 481 for trusted sources with no transaction — §4.1 step 8.)
...
passive/traffic state remains as a safety net behind the probe. With the peer
reply hand-off on (§8.12, `SBC_PEER_INTERNAL_IP`), the active hands those
transaction-less OPTIONS replies back to the standby, whose 6/7 then read
Active too.
...
| `SBC_PEER_INTERNAL_IP` | No (unset = compiled out) | 10.142.0.101 | The OTHER SBC's VPC IP (sbc-1 → sbc-2 and vice versa). Enables the peer reply hand-off (§8.12): `#!define SBC_PEER_HANDOFF`, `peerhealth` htable, dispatcher group 10 liveness probe. Must be a dotted-quad IPv4 and differ from `SBC_INTERNAL_IP` (else WARNING, treated as unset). Set on BOTH SBCs. |
...
### 8.11 Post-failover teardown (active/standby flip) — what an operator sees

Each zone's SBC pair is active/standby by NLB health check (identical config on both; `db_mode=0`, no DMQ — dialog and transaction state is per-SBC and never replicated). After a flip:

- **Established calls tear down on EITHER SBC.** Carrier-side BYE/ACK/re-INVITE are dispatched statelessly by the `;fs=` Route marker (§8.10); FS-side BYEs (carrier-bound AND trunk-PBX-bound) are `forward()`ed statelessly in the WITHINDIALOG tail. No dialog lookup is on the call path, so the new active needs nothing from the old one.
- **Ghost dialog entries on the ex-active age out at `default_timeout=7200` (2h).** Every call the ex-active owned at the flip never sees its BYE there (it landed on the new active), so `dlg.stats_active`, `kamailio_dialog_active_dialogs` and the `carrier_trunk` profile gauges on THAT SBC over-read for up to 2h, then self-correct. Gauges/shm only — expiry is silent (no `send_bye`, by design: the call is over or was torn down via the peer; a generated BYE would be wrong). 7200 = 2× the largest Session-Expires we ever accept (3600 on the 422 retry; normal 1800, refresh ~900s), and the dialog module re-arms the timer on every sequential request (`timeout_noreset=0` default — carrier re-INVITEs via `dlg_manage()` in FROM_NET_INDIALOG, FS re-INVITEs via the rr callback in `loose_route()`), so a live long call never expires from bookkeeping; FS `max-duration=43200` bounds real calls. `timeout_avp` (`$avp(dlg_timeout)`) is declared but never set — all dialogs use the default.
- **Setups in flight at the flip are lost** (standard HA semantic). The carrier's CANCEL for such an INVITE reaches the new active with no transaction: it now gets `481` (flag-5 trusted sources only) instead of a silent drop, so the carrier stops retransmitting immediately and the forwarded leg stops phantom-ringing. FS-side B-leg setups in flight fall through to `inbound_router.lua`'s `progress_timeout` failover loop.
- **Verify after a drill:** `kamcmd dlg.stats_active` on the ex-active decays to ~0 within 2h; Homer shows a single `481` from the new active to the carrier's CANCEL, no CANCEL retransmit train.

### 8.12 Peer reply hand-off (active/standby pair) — `SBC_PEER_INTERNAL_IP`, default off

**Problem.** Carrier replies to VIP-sourced requests (VIP-egress INVITE/CANCEL, the group-6/7 VIP-sourced Sinch OPTIONS probes) return through the external NLB to whichever SBC is NLB-ACTIVE. In steady state that is the sender; during **failback** (ex-active still alive with in-flight transactions) or a **maintenance drain** it is the OTHER SBC, which has no tm transaction: core stateless-forwards the reply to FS by via2 (raw — no REPLY_HANDLER, no Session-Expires normalization) while the owner's tm never sees it → Timer A retransmits, fr_inv_timer → local 408 → `failure_route[CARRIER_FAILURE]` re-INVITEs the alternate PoP with the same Call-ID → duplicate ring at the callee. The standby's VIP-sourced OPTIONS replies are dropped the same way ("no 2nd via"), so it reads Sinch orig groups 6/7 Inactive forever.

**Mechanism** (`reply_route`, inside the existing `!t_check_trans()` branch after `sip_trace()`; compiled in only by `#!define SBC_PEER_HANDOFF`): when `$Ri == ADVERTISE_IP` (received on the external VIP bind) AND `!route(IS_INTERNAL_SOURCE)` AND `$rm` ∈ {INVITE, CANCEL, OPTIONS} AND `$sht(peerhealth=>up) == 1`, `send_data("sip:SBC_PEER_IP:5060", "$mb")` ships the received datagram VERBATIM (corex `udp_send` of `$mb` = `msg->buf`; no Via touched) to the peer's VPC IP and `drop` stops core's own via2 forward. The peer's tm matches it by the top-Via branch (`t_reply_matching` keys on hash/md5/branch + CSeq method and never reads the packet source or receive socket) and processes it exactly as if the carrier had sent it there. `send_data` — NOT `send_udp`, which is `REQUEST_ROUTE|FAILURE_ROUTE` only and fails `kamailio -c` in reply_route. All 5.8.8 file:line citations are in the cfg comment.

**Method scope.** INVITE (the duplicate-ring case), CANCEL (stops tm's CANCEL retransmit train), OPTIONS (probe replies → the standby's dispatcher reads real carrier health). **BYE excluded on purpose:** FS→carrier BYEs are stateless-forwarded (§8.10 WITHINDIALOG tail) — no transaction on either SBC — a hand-off would only add a hop.

**Liveness gate.** Dispatcher group 10 (`sip:SBC_PEER_IP:5060`, NIC-sourced OPTIONS, same probing as group 1) is mirrored into `peerhealth=>up` by `event_route[dispatcher:dst-up|dst-down]` exactly like `fshealth` (matched on `$ru` — the 5.8 event-route gotcha; exact-URI match; seeded 1 at mod-init). A UDP send to a dead host does not fail, so without the gate a reply would simply vanish; peer DOWN ⇒ fall through to today's stateless via2 forward — the raw copy that lets a setup survive an SBC death mid-INVITE. Residual: the probe detection window after a SILENT peer death (~40-45s, fr_timer-bound; ~10-15s if it answers with an error) still hands those replies to the dead peer. The peer answers our OPTIONS from an internal source with 200 regardless of its own FS health (the FS-aware 503 arm is `!IS_INTERNAL_SOURCE`) — correct, its tm is what must be alive. Group 10 is NOT a carrier: carrier-monitor `CARRIER_SETIDS` excludes it; its flaps only bump the `dsmon` gen counter (one extra snapshot).

**Loop safety.** The copy reaches the peer from our `SBC_INTERNAL_IP` ⇒ `IS_INTERNAL_SOURCE` ⇒ the guard cannot fire again (and `$Ri` there is the NIC bind, not the VIP); a peer with no transaction either does today's via2 forward. A `send_data` failure also falls through (WARN).
...
- Dialog: `default_timeout=7200` (2h ghost-entry bound, NOT a max call length — the timer re-arms on every sequential request; §8.11), no DB persistence (stateless SBC)
