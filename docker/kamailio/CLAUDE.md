...
### 8.10 ⚠️ KNOWN DEFECT (HOLD — not yet fixed): carrier-originated in-dialog BYE `481`s when the NLB delivers it to the non-owner SBC

**Status:** Root-caused + fix designed, but **deliberately NOT applied yet** (owner decision, 2026-08-02 — this is the single most critical file in the platform, live in all 3 zones, so it's being staged deliberately). Affects **all three zones** (config is byte-identical; only env values differ). Discovered on West via HEP capture of a real call (A-leg dialog `did=b3f.824`/ftag `gK00149a7b` owned by west-sbc-2 `10.138.0.101`; B-leg `did=a96.58d2`/ftag `4ZK8K1DKNe1Ba` owned by west-sbc-1 `10.138.0.100`).
