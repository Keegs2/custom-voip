...
- **`sip_code` precedence** (`_derive_sip_code`): `sip_term_status` → `proto_specific_hangup_cause` (`sip:N`) → `last_bridge_proto_specific_hangup_cause` (`sip:N`, the B-leg's terminating status mirrored onto the A-leg at partner hangup) → **200 if answered** (`answer_epoch > 0`; the INVITE completed 200 OK and FS sent the BYE, so no leg-local status exists — this was the "NULL on most answered calls" bug) → `sip_invite_failure_status` → NULL. `sip_term_status` is only set on the leg that RECEIVED the BYE (`sofia.c:1036`, `sofia_handle_sip_i_bye`) or whose dialog terminated with a status (`sofia.c:8743`, `nua_callstate_terminated`), and is never mirrored to the partner. `last_bridge_proto_specific_hangup_cause` is the bonded partner's `proto_specific_hangup_cause` pushed across in `switch_channel.c:3432-3434` (name assembled by token pasting, so grepping the literal finds nothing). `sip_invite_failure_status` is set on BOTH the failing leg and its partner (`sofia.c:6657-6660`) but the success branch (`:6681-6682`) clears it on the PARTNER ONLY — it can be stale on the local leg, which is exactly why it sits below the `answered` rule.
