...
- **One model, one grade.** `services/call_quality.py` (pure, stdlib-only; also imported read-only by the FS lab `docker/freeswitch/lab/eval_cdrs.py`) is the source of truth: ITU-T G.107 E-model (packet-loss impairment only; `R0=93.2`, `ID=0`, G.711 `Ie=0/Bpl=25.1`, G.729 `11/19`, unknown codec → G.711), MOS from R (Annex B), `grade_for_mos` great ≥ 4.34 · good ≥ 4.02 · fair ≥ 3.60 · poor (on the STORED 2-dp MOS). Clean G.711 = **4.41**, not 4.50. SQL mirrors `cq_r_factor` / `cq_mos` / `cq_grade` / `cq_grade_rank` / `cq_leg_status` (migration 50) are float8-identical — `tests/test_cdr_quality_migration50.py` enforces parity (10k random points). `services/reporting.grade_for_mos` delegates. Never add a second threshold set.
- **Rating rule (per leg, A and B rows):** unanswered → `unanswered`; no `rtp_audio_in_packet_count` → `no_data`; billable < 5 s → `short`; inbound packets < 10 % of `billable_ms/ptime` → `no_rtp` (one-way audio: grade **poor, MOS NULL**); < 250 packets → `low_sample`; loss input missing → `no_data`; else `rated`. Only `rated` rows carry mos/r_factor/loss/jitter/burst_ratio; everything else is NULL (0 = measured zero).
...
- **Reports** read `call_mos AS mos, call_quality_grade AS grade`; graded ⇔ grade set; `no_rtp` → "Poor" in the CSV.
- **Deploy:** migration 50 BEFORE the API build, then `docker/postgres/backfill/50_cdr_quality_backfill.psql` (see `docker/postgres/CLAUDE.md`). Alerting: `scripts/backup/media_guard.sh` (one-way-audio spike → `revup-alert`).
