...
- **Deploy:** migration 50 BEFORE the API build, then `docker/postgres/backfill/50_cdr_quality_backfill.psql`; migration 51 + `backfill/51_reclassify_no_media.psql` (see `docker/postgres/CLAUDE.md`). No alerting/paging on quality (owner decision 2026-09-24): one-way audio is visible on the dashboards only.
