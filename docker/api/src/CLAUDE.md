...
- **Ingest**: Parses FreeSWITCH JSON CDR format with robust field extraction:
  ...
  - Quality (migration 50, `services/call_quality.py`): mos / r_factor come from OUR ITU-T G.107 E-model over TRUE loss (patched FS `rtp_audio_in_seq_*` vars, else `rtp_audio_in_jitter_loss_rate` fraction); per-leg rating rule (unanswered / short / no_rtp / low_sample / no_data / rated); call-level worst-direction via `cdr_refresh_call_quality()`. Raw FS values kept in `fs_*` columns. See docs/CALL_QUALITY_ACCURACY_PLAN.md.
