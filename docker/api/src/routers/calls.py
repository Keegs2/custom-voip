...
RETIRED PRODUCT (2026-09) — code kept, switched OFF. This router is only
mounted by main.py when ``API_CALLING_ENABLED`` is exactly "true"
(``config.api_calling_enabled``), and every route additionally carries the
``require_api_calling_enabled`` dependency (404 when off) as defense in depth.
...
The machine-payments demo (x402 pay-per-call on this endpoint, the ledger-posted
per-call fee, and the PAYMENTS_DEMO_FAKE_ORIGINATE short-circuit) was REMOVED
with the payments demo — its x402 "facilitator" was a simulation that accepted
any signature, so it must never ride a production flag. What remains is the
exact flag-off legacy flow: prepaid originate + raw balance decrement.
"""
...
import uuid
import logging
from config import require_api_calling_enabled
...
router = APIRouter(dependencies=[Depends(require_api_calling_enabled)])
