"""Append-only ledger service — the monetary spine (payments foundation).

Design: docs/PAYMENTS_SYSTEM_DESIGN.md §0-§2 (ported from the unified branch's
Wave 1). Backed by ``docker/postgres/init/37_payments_ledger.sql``.

Every money event (topup, usage, fee, refund, adjustment, promo, chargeback)
posted here is recorded as an immutable row in ``ledger_entries``, and the
``customers.balance`` column is updated in the SAME transaction as the entry
insert, so for customers whose balance moves exclusively through this module::
...
║  RCF/trunk billing are deliberately NOT rewired through this ledger       ║
║  (unlike unified's Wave 1). ``customers.balance`` is live prod-shaped     ║
║  data for real customers, so the balance-as-cache reconciliation          ║
║  invariant above holds ONLY for demo customers and future API-calling     ║
║  charge flows — never assume it for a real RCF/trunk customer.            ║
╚══════════════════════════════════════════════════════════════════════════╝
...
# Mirrors the CHECK constraints in docker/postgres/init/37_payments_ledger.sql.
...
async def _post_in_conn(
    ...
    *,
    ...
    # 2) Atomically move the balance and capture the resulting balance.
    #    RCF-V1: only demo / API-calling charge flows reach here (see module
    #    docstring). Uses explicit ::type casts for asyncpg/PgBouncer.
    ...
    *,
    ...
    The FIRST positional arg is either:
      * an ``asyncpg.Connection`` already inside a transaction — the entry insert
        and balance update join the CALLER's transaction (use this from the
        demo topup/settlement paths so the whole operation is atomic with any
        sibling writes). When you pass a connection you MUST also pass
