-- ==========================================================================
-- 21_account_cleanup.sql   (renamed from Full-System's 17_account_cleanup.sql)
--
-- !!! DEV / DEMO ONLY — DO NOT RUN IN PRODUCTION !!!
-- This is a destructive demo-data reset. It DELETEs customers with IDs 1-5
-- (and their cascaded child records), renames "Granite Engineering" to
-- "Granite Telephony", and seeds presentation demo accounts/users/trunks.
-- It MUST be excluded from the production init set (e.g. leave it out of the
-- production init mount, or guard it behind a dev-only env flag).
--
-- Renumbered from 17 -> 21 so it does NOT collide with RCF-V1's
-- 17_did_inventory.sql and so it runs LAST (after 14/16/17/18/19/20),
-- AFTER all schema + Granite seed files have created the parent tables it
-- depends on. The docker entrypoint runs init scripts alphabetically; at
-- prefix 17 the original name (17_account_cleanup) sorted BEFORE
-- 17_did_inventory and ahead of 18/19/20, which would wipe customers 1-5
-- before those scripts ran. The 21 prefix fixes that ordering.
--
