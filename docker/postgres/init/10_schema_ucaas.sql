...
--
-- FK-SAFETY (RCF-V1 unify): customer id=5 is created by 11b_add_ucaas_type.sql,
-- which runs AFTER this file. On the hardened RCF-V1 base the old test
-- customers (1-5) were stripped out of 06_seed_data.sql, so customer 5 does
-- NOT exist yet at this point. A bare INSERT here would FK-violate and abort
-- the whole init. We guard on customer 5's existence; 11b re-seeds these exact
-- extensions (ON CONFLICT DO NOTHING) after it creates customer 5, so nothing
-- is lost. On legacy bases where 06 still seeds customer 5, this still runs.
-- ---------------------------------------------------------------------------
INSERT INTO extensions (extension, customer_id, display_name)
SELECT v.extension, v.customer_id, v.display_name
FROM (VALUES
    ('2001', 5, 'UCaaS User 1'),
    ('2002', 5, 'UCaaS User 2')
) AS v(extension, customer_id, display_name)
